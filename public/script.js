...
function showChat() {
  ...
  html+='<br/>';
  html+='<input id="msg" type="text" placeholder="Send message...">';
  html+='<button id="send">Send</button>';
  html+='<br/>';
  html+='<button onclick="$(\'#chat\').empty()">Clear Chat</button>';
  html+='<button onclick="$(\'#drawing\').empty()">Delete Drawing</button>';
  html+='</div>';
  $("body").append(html);
  $("#send").on("click", () => {
